...
public class ListPaginator<E, M> {
    ...
    /**
     * Adds a callback method which is invoked when the passed list in {@link #sendPage(List, CommandSender, int, PageFooter)}
     * is empty.
     *
    ...
    /**
     * Adds a callback method when the page index passed in {@link #sendPage(List, CommandSender, int, PageFooter)} is invalid (i.e less than 0
    ...
    /**
     * Returns the amount of pages this list contains. It divides by the page, and if there are any
     * remaining elements (i.e remainders) they get their own page (which adds by 1).
     *
     * @param list List to get the size for
     * @return The pages count this list can hold
     */
    public int getPageSize(Collection<?> list) {
     ...
     */
    public void sendPage(List<E> list, CommandSender target, int pageNumber, @Nullable PageFooter footer) {
        if (list.isEmpty()) {
            if (ifEmpty != null)
        ...
        if (pageNumber > size) {
            if (invalidPage != null) {
                ...
                throw new IllegalArgumentException(String.format("Page number %s is greater than the pages size %s", pageNumber, size));
            }
        }
        if (pageNumber <= 0) {
            if (invalidPage != null) {
                ...
                throw new IllegalArgumentException(String.format("Page number %s invalid", pageNumber));
            }
        }
